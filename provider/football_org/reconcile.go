...
func Reconcile() error {
	...
	now := time.Now()
	matches, err := repository.FindMatchesToReconcile(entity.FootballOrg, now.Add(RECONCILE_INTERVAL), now)
	if err != nil {
		return err
	}
	...
	for _, match := range matches {
		...
		if err := repository.FinishMatch(match.ID, apiResponse.Score.FullTime.Home, apiResponse.Score.FullTime.Away); err != nil {
